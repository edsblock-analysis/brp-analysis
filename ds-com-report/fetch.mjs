// Fetch all ds-com.txt URLs with bounded concurrency; cache raw HTML.
...
const RAW = fs.readFileSync(path.join(ROOT, 'ds-com.txt'), 'utf8')
...
async function fetchOne(u) {
  ...
  if (fs.existsSync(file) && fs.statSync(file).size > 500) { done += 1; return { url: u, status: 'cached', bytes: fs.statSync(file).size }; }
...
async function run() {
  ...
  async function worker() { while (queue.length) { const [u, i] = queue.shift(); results[i] = await fetchOne(u); if (done % 50 === 0) process.stderr.write(`  ${done}/${URLS.length}\n`); } }
